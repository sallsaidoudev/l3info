...
(** Moves bullet adequately to its specific behavior. Checks if bullet has hit
   something using General.move_slowly function. If yes - collision function is called, 
   which responds adequately (most likely creates explosion and deletes bullet *)
...
(** Throws given number of debris pieces on given position with given
   initial velocity *)
val generate_debris : int * int -> float * float -> int -> unit
